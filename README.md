...
- ✅ 評価項目管理（大項目・中項目・説明の構造）
  - 登録、削除、編集、表示順設定（自動計算：最大値+1）
  - 編集モードでリアルタイム変更
- ✅ 採点調整フォーム（**表形式表示：横軸=評価項目、縦軸=被評価者**）
  - 評価者別フィルター機能
  - リアルタイム点数更新（変更時に自動保存、背景色フィードバック）

### 使用者機能（登録されたユーザー名/パスワードでログイン）
- ✅ 採点フォーム（**表形式表示：横軸=評価項目、縦軸=メンバー名**）
  - 一括保存機能（全員分まとめて保存）
  - 自己評価対応
  - 大項目・中項目・説明を表示
- ✅ 入力結果表示（自分が入力した評価の一覧）
- ✅ 集計結果表示（チーム内メンバーの平均評価点）
- ✅ **月次推移表示**（過去の評価結果との比較、増減トレンド表示）

### 認証機能
- ✅ 管理者ログイン（ID: admin / パスワード: admin）
- ✅ 使用者ログイン（ユーザー名/パスワード、初期パスワード=ユーザー名）
- ✅ セッション管理
...
### 評価データAPI
- `GET /api/team-members` - 自チームのメンバー取得（自己評価含む）
...
- `GET /api/evaluations/monthly` - **月次集計結果**（過去データ含む）
...
### 使用者の使い方
1. 管理者に登録されたユーザー名/パスワードでログイン（初期パスワード=ユーザー名）
2. **採点フォーム**: 
   - 表形式で自チームのメンバー（自己評価含む）を各項目で1～10点評価
   - 全て入力したら「全て保存」ボタンで一括保存
3. **入力結果**: 自分が入力した評価内容を確認
4. **集計結果**: チーム内メンバーの平均評価点を確認
5. **月次推移**: 過去の評価結果と比較（増減トレンド表示）
...
- **管理者**: admin / admin
- **チームA**: 
  - user001 / user001（田中太郎）
  - user002 / user002（佐藤花子）
  - user003 / user003（鈴木一郎）
- **チームB**: 
  - user004 / user004（高橋美咲）
  - user005 / user005（伊藤健太）
  - user006 / user006（渡辺由美）
